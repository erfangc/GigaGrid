...
describe("a tree builder", () => {
    ...
    it("should take four flat rows of data, a SubtotalBy object and turn it into a deep tree structure", () => {
        ...
        expect(tree.getRoot().children[0].name).toBe("A");
        expect(tree.getRoot().children[1].name).toBe("B");

        expect(tree.getRoot().children[0].children[0].name).toBe("C");
        expect(tree.getRoot().children[1].children[0].name).toBe("C");

        expect(tree.getRoot().children[0].children[1].name).toBe("D");
        expect(tree.getRoot().children[1].children[1].name).toBe("D");
